...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 521, *, text="@everyone Easy"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 100, *, text="@everyone Easy"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
